...
⚠️ Make sure you done editing `package.json` and files in `Packages/com.ivanmurzak.unity.mcp.terrain` folder. Because it is going to be public with no ability to discard it.

1. Increment package version in the file `Packages/com.ivanmurzak.unity.mcp.terrain/package.json`. It has `version` property.
